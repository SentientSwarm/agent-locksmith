...
| `value: { from_vault: ... }` | Post-v2 only — stub today | `BackendError::NotImplemented`; tracked outside v2 |
| `value: { from_aws_secrets_manager: ... }` | Post-v2 only — stub today | Same |

The acceptance bar for v0.6.0 production deployments is `from_file_sealed`. See §3 for the recipe.
